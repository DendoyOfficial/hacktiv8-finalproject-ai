...
async function askGemini(text) {
  appendBaristaResponse('...');
  const typing = dynamicMessages.lastElementChild;
  ...
  try {
    ...
    typing.remove();
    appendBaristaResponse(esc(reply).replace(/\n/g, '<br>'));
    if (res.ok) history.push({ role: 'user', text }, { role: 'model', text: reply });
  } catch (e) {
    typing.remove();
    appendBaristaResponse('Maaf, koneksi bermasalah. Coba lagi ya!');
  }
}

window.sendQuickReply = function (text) {
  appendUserMessage(esc(text));
  askGemini(text);
};

window.openChatWithContext = function (text) {
  toggleChat(true);
  setTimeout(() => window.sendQuickReply(text), 400);
};
